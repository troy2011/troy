...
const DEMO_PLAYFAB_ID = 'PERSONALITY_DEMO';
const TERMINAL_TOKEN = process.env.PERSONALITY_ASSESSMENT_TERMINAL_TOKEN
    || 'troy-local-personality-demo-terminal';
const SIGNING_SECRET = process.env.PERSONALITY_ASSESSMENT_SIGNING_SECRET
...
function startDemo() {
    ...
    initializePersonalityAssessmentRoutes(app, { firestore, ...playFab }, {
        enabled: true,
    ...
    const server = app.listen(PORT, HOST, () => {
        const url = `http://${HOST}:${PORT}/tarot-reading.html?personalityTerminal=${encodeURIComponent(TERMINAL_TOKEN)}`;
        console.log(`[personality-demo] ${url}`);
        console.log('[personality-demo] 結果はメモリ内だけに保存され、終了時に破棄されます。');
    });

    server.on('error', (error) => {
        console.error('[personality-demo] 起動できませんでした:', error.message);
        process.exitCode = 1;
    });
}
