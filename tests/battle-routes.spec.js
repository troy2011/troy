...
test('exploration npc battle persists a numeric speed stat for naval NPCs', async () => {
    ...
    if (fn === PlayFabServer.GetPlayerStatistics) {
      return {
        Statistics: [
          ...
          { StatisticName: 'Power', Value: 28 },
          { StatisticName: 'Defense', Value: 12 },
          { StatisticName: 'Agi', Value: 14 },
          { StatisticName: 'Speed', Value: 14 }
        ]
      };
    }
  ...
  try {
    ...
    expect(npcPlayer.stats.すばやさ).toBeGreaterThan(0);
  } finally {
    Math.random = originalRandom;
    delete require.cache[battleRoutesPath];
