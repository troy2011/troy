...
test('gold plaque banner images are auto-upgraded wherever reused', async ({ page }) => {
  await page.route('**/main.js*', (route) => route.abort());
  await page.goto('/index.html');
  await page.setContent(`
    <!doctype html>
  ...
  await expect(page.locator('.panel-slice-25-layer[data-source="banner-plaque-gold.png"] > .panel-slice-25-cell')).toHaveCount(75);
  ...
  const plaqueMetrics = await plaques.evaluateAll((elements) => elements.map((element) => {
    ...
    return {
      ...
      fillCount: element.querySelectorAll(':scope > .panel-slice-25-layer > .panel-slice-25-fill').length,
      cellCount: element.querySelectorAll(':scope > .panel-slice-25-layer > .panel-slice-25-cell').length
    };
  }));
  ...
  plaqueMetrics.forEach((metrics) => {
    ...
    expect(metrics.cellCount).toBe(25);
  });
});
