...
test('Tarot Kingdom shares the canonical inventory module with the app shell', () => {
  ...
  expect(indexSource).toContain('"inventory": "./js/inventory.js?v=20260815-ap-status-row-v1"');
  expect(previewSource).toContain('"inventory": "./js/inventory.js?v=20260815-ap-status-row-v1"');
});
...
test('Tarot Kingdom release advances its entry modules and service-worker cache together', () => {
  const release = '20260815-ap-status-row-v1';
  const indexSource = fs.readFileSync(path.join(ROOT_DIR, 'public', 'index.html'), 'utf8');
  ...
  expect(indexSource).toContain('style.css?v=20260815-ap-status-row-v1');
  ...
  expect(previewSource).toContain('./style.css?v=20260815-ap-status-row-v1');
  expect(serviceWorkerSource).toContain("const CACHE_VERSION = 'troy-app-v20260815d';");
});
